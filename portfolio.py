...
# REAL >> floating point, TEXT >> str
# execute performed on cursorObj 
cursorObj.execute("CREATE TABLE IF NOT EXISTS coin(id INTEGER PRIMARY KEY, symbol TEXT, amount INTEGER, price REAL)")
# to save changes, commit performed on con (object of connection)
con.commit()

def reset():
    # destory every tkinter elements to not have so many frames when refresh
...
# put all inside a fun. to call api only when needed 
def my_portfolio():
    ...
    # return list of tuples , each tuple represent a row
    ...
    def insert_coin():
        ...
        # call it, to refresh auto(to see the updates), and not enter the refresh button to 
        reset()
    ...
    for i in range(0, 300):
      for coin in coins:
        if api["data"][i]["symbol"] == coin[1]:
          # total paid = coin['amount_owned/ num of coin'] * coin['price_per_coin']
          # with old price (my portfolio price)
          total_paid = coin[2] * coin[3]
          # wirh current price 
          current_value = coin[2] * api["data"][i]["quote"]["USD"]["price"]
          # profit loss per coin: current price - old price
          pl_percoin = api["data"][i]["quote"]["USD"]["price"] - coin[3]
          # total profit for all coins
          total_pl_coin = pl_percoin * coin[2]
           
          # total for all portofilio 
          total_pl += total_pl_coin
          total_current_value += current_value
          total_amount_paid += total_paid
          
          
	  # viewing data using TK instade of print, from second row and so one
	  ...
	  # font is green if profit is positive , else red
    ...
    # to clear all api data , and call it next time with new get 
    api = ""

    # to refresh the portfolio with the api data
    # the api is updated every 5 mins. , so the button only update according to it
...
def app_header():
    ...
    # fg >> font color, font = 'name size style' 
    ...
    # grid to postition items using cols ans rows
    # sticky >> allignment method (south east west north)
    # relief="groove" >> effects around the outside of the widget.
...
app_header()
my_portfolio()
# must be last to execute all the preveious code first, then enter loop
pycrypto.mainloop()
